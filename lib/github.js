...
export function createGitHubClient({
  ...
  async function attemptOnce({ method, path, query, body, url, accept, redirect, signal, headers: extraHeaders, rawText }) {
    ...
    try {
      const requestBody = (() => {
        ...
        if (typeof body === 'string' || (typeof Buffer !== 'undefined' && Buffer.isBuffer(body)) || body instanceof Uint8Array) {
          return body
        }
  ...
  async function call({ method = 'GET', path, query, body, url, accept = 'application/vnd.github+json', redirect, signal, headers, rawText }) {
    ...
    for (let attempt = 1; attempt <= attemptsAllowed; attempt += 1) {
      let result
      try {
        result = await attemptOnce({ method: upper, path, query, body, url, accept, redirect, signal: turnSignal, headers, rawText })
      } catch (err) {
        lastError = err
  ...
  return {
    ...
    baseUrl,
    call,
    getRaw: (path, opts = {}) => call({ ...opts, method: 'GET', path, rawText: true }).then((r) => r.data),
