...
import { registerApprovalGate, ALL_ACTIONS } from './approval.js'
...
/** Actions this plugin can be asked to perform; exposed for settings validation. */
export const KNOWN_ACTIONS = ALL_ACTIONS
...
export const Config = Schema.object({
  ...
  approvalMode: Schema.string().default('auto')
    .description('"auto" (default): an agreed, non-destructive write goes through without a prompt, while deleting a release, tag, secret, variable, ruleset, branch protection or cancelling a run always asks. "ask": every write asks. "off": the plugin has no opinion and the host approval contour decides.'),
  allowedActions: Schema.array(Schema.string()).default([])
    .description('Write actions this plugin may perform; anything else is denied. Empty means every known action is allowed, and each one still asks for confirmation.'),
  autoApprove: Schema.array(Schema.string()).default([])
    .description('Actions an unattended run (DSH_GITHUB_OPS_UNATTENDED=1) may perform without asking. Destructive actions are never auto-approved.'),
...
export function apply(ctx, rawConfig) {
  ...
  const deps = { tool, asRepo, client, liveConfig, reviewRules }
  ...
  // the write still goes through the approval gate.
  ...
  // Writes go through the approval contour: a named action, a reason a human can decide
  // on, and no auto-approval for anything destructive.
  try {
    registerApprovalGate(ctx, () => liveConfig())
  } catch (err) {
    ctx.logger?.warn?.('[dsh-github-ops] approval gate not registered:', err && err.message)
  }
