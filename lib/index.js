...
import { resolveAccess, NO_ACCESS_GUIDANCE, createClientHolder } from './auth.js'
...
export function apply(ctx, rawConfig) {
  ...
  // current tool call arrives through the async context. Token rotation immediately recreates
  // the client even when the new token has the exact same length.
  const clientHolder = createClientHolder({
  ...
  async function client() {
    ...
    return clientHolder.get(access, cfg)
  }
  ...
  const catalog = []
  const WRITE_PREFIXES = ['pr_', 'issue_', 'review_post', 'gh_api', 'gh_run_rerun', 'gh_run_cancel', 'gh_run_dispatch', 'gh_mirror_publish', 'gh_release_', 'gh_tag_', 'gh_repo_create', 'gh_repo_edit', 'gh_secret_', 'gh_variable_', 'gh_ruleset_', 'gh_branch_protection_set', 'gh_branch_protection_delete', 'gh_push_']
      ...
      execute: async (args, exec) => {
        try {
          ...
          if (WRITE_PREFIXES.some((prefix) => toolName === prefix || toolName.startsWith(prefix))) {
            try {
              clientHolder.currentClient()?.cacheClear?.()
            } catch {
  ...
  ctx.inject(['webServer'], (wctx) => {
    try {
      const mount = () => registerPluginUpdater(wctx, {
        endpoint: '/dsh-github-ops/update',
        packageName: '@goodandready/dsh-github-ops',
        manifestUrl: new URL('../package.json', import.meta.url),
      })
      if (typeof wctx.effect === 'function') wctx.effect(mount, 'dsh-github-ops: updater')
      else mount()
    } catch (err) {
      ctx.logger?.warn?.('[dsh-github-ops] updater route not registered:', err && err.message)
    }
  })
  ...
  registerRoutes(ctx, {
    ...
    describeError,
    makeGitRunner,
    cacheStats: () => clientHolder.currentClient()?.cacheStats?.() || null,
  })
