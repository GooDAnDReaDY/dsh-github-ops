...
function findingsFor(files = []) {
  ...
  const touches = (re) => paths.filter((p) => re.test(p))
  const bigDiff = files.reduce((sum, f) => sum + (f.additions || 0) + (f.deletions || 0), 0)

  const ci = touches(/^\.github\/workflows\/|^\.gitea\/workflows\//)
  if (ci.length) findings.push({ level: 'attention', area: 'ci', detail: `workflow files changed: ${ci.join(', ')}` })

  const migrations = touches(/migrations?\//i)
  if (migrations.length) findings.push({ level: 'attention', area: 'migration', detail: `database migrations touched: ${migrations.join(', ')}` })

  const secrets = touches(/(^|\/)(\.env|\.npmrc|\.credentials|id_rsa)/)
  if (secrets.length) findings.push({ level: 'critical', area: 'secrets', detail: `possible secret material in the diff: ${secrets.join(', ')}` })

  const docs = touches(/(^|\/)(AGENTS\.md|index\.md)$/)
  if (docs.length) findings.push({ level: 'attention', area: 'internal-docs', detail: `internal agent files changed: ${docs.join(', ')}` })

  const code = files.filter((f) => /\.(js|mjs|cjs|ts|tsx|py|go|rs|java)$/.test(f.filename))
  const tests = files.filter((f) => /(^|\/)(test|tests|spec)\b|\.test\.|_test\./.test(f.filename))
  if (code.length && !tests.length) {
    findings.push({ level: 'attention', area: 'tests', detail: `${code.length} source file(s) changed with no test change` })
  }
  if (bigDiff > 800) findings.push({ level: 'attention', area: 'size', detail: `large diff: ${bigDiff} changed lines` })
...
/** One review pack: everything a reviewer needs, with the diff capped. */
export async function reviewPull(client, { owner, repo, number, maxDiffChars = 60000 }) {
  ensureRepo({ owner, repo })
  if (!number) throw new Error('number is required')
 ...
 */
export async function ciRun(client, { owner, repo, number, maxDiffChars }) {
  const pack = await reviewPull(client, { owner, repo, number, maxDiffChars })
  const blocking = pack.findings.filter((f) => f.level === 'critical')
  const attention = pack.findings.filter((f) => f.level === 'attention')
  const checksFailed = pack.checks && pack.checks.rollup === 'failure'
  const verdict = blocking.length || checksFailed ? 'needs-changes' : 'success'
  return {
    verdict,
