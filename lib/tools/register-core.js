...
import { listReleases, getRelease, createRelease, editRelease, deleteRelease, listReleaseAssets, uploadReleaseAsset, deleteReleaseAsset, generateReleaseNotes } from './releases.js'
...
export function registerCoreTools({ tool, asRepo, client, liveConfig }) {
  ...
  tool(
    'gh_release_edit',
    'Edit a GitHub release: title, notes, draft/prerelease flags, and which release is marked latest (makeLatest). Use makeLatest: true after publishing a new version so the repository badge does not stay on an older release. For a DRAFT release pass releaseId (returned by gh_release_create): GitHub hides drafts from the by-tag lookup.',
    repoParams({
      tag: { type: 'string', description: 'Release tag to edit.' },
      releaseId: { type: 'number', description: 'Release id from gh_release_create or gh_release_list; required for a draft.' },
  ...
  tool(
    'gh_release_delete',
    'Delete a GitHub release. Requires confirm: true. The git tag itself is not removed — use gh_tag_delete for that. For a DRAFT release pass releaseId: GitHub hides drafts from the by-tag lookup.',
    repoParams({
  ...
  tool(
    'gh_release_asset_list',
    'List uploaded assets of a GitHub release (by tag or releaseId). Read-only.',
    repoParams({
      tag: { type: 'string', description: 'Release tag.' },
      releaseId: { type: 'number', description: 'Release ID.' },
      limit: { type: 'number', description: 'Max assets to return (default 50).' },
    }),
    async (args) => {
      const { owner, repo } = await asRepo(args, liveConfig())
      return listReleaseAssets(await client(), { owner, repo, tag: args.tag, releaseId: args.releaseId, limit: args.limit })
    },
  )

  tool(
    'gh_release_asset_upload',
    'Upload a file asset to an existing GitHub release (by tag or releaseId). Requires confirm: true.',
    repoParams({
      tag: { type: 'string', description: 'Release tag.' },
      releaseId: { type: 'number', description: 'Release ID.' },
      filePath: { type: 'string', description: 'Local path to the file to upload.' },
      name: { type: 'string', description: 'File name as it will appear in the release (defaults to file basename).' },
      label: { type: 'string', description: 'Optional user-facing label description for the asset.' },
      contentType: { type: 'string', description: 'MIME type of the asset (defaults to application/octet-stream).' },
      confirm: { type: 'boolean', description: 'Must be true to upload.' },
    }),
    async (args) => {
      if (args.confirm !== true) throw new Error('uploading a release asset requires confirm: true')
      const { owner, repo } = await asRepo(args, liveConfig())
      return uploadReleaseAsset(await client(), {
        owner, repo,
        tag: args.tag,
        releaseId: args.releaseId,
  ...
  tool(
    'gh_release_asset_delete',
    'Delete an asset from a release by assetId. Requires confirm: true.',
    repoParams({
      assetId: { type: 'number', description: 'ID of the asset to delete.' },
      confirm: { type: 'boolean', description: 'Must be true to actually delete.' },
    }),
    async (args) => {
      if (args.confirm !== true) throw new Error('deleting a release asset requires confirm: true')
      const { owner, repo } = await asRepo(args, liveConfig())
      return deleteReleaseAsset(await client(), { owner, repo, assetId: args.assetId })
    },
  )

  tool(
    'gh_release_notes_generate',
    'Generate release notes for a release tag from merged pull requests and commits. Read-only.',
    repoParams({
      tag: { type: 'string', description: 'Tag name for the release (e.g. "v1.2.0").' },
      target: { type: 'string', description: 'Target branch or commit if tag does not exist yet.' },
      previousTag: { type: 'string', description: 'Previous tag to compare against.' },
      configFile: { type: 'string', description: 'Path to configuration file in repo (e.g. ".github/release.yml").' },
    }),
    async (args) => {
      const { owner, repo } = await asRepo(args, liveConfig())
      return generateReleaseNotes(await client(), {
        owner, repo,
        tag: args.tag || args.tagName,
        target: args.target || args.targetCommitish,
        previousTag: args.previousTag || args.previousTagName,
        configFile: args.configFile || args.configurationFilePath,
      })
    },
  )
  ...
  tool(
    ...
    'Call the GitHub API directly for anything the other tools do not cover (refs, rulesets, orgs, gists). Reads are free; POST/PATCH/PUT/DELETE require confirm: true, and deleting a repository, transferring one, or deleting an organization is refused outright. Pass graphql to run a GraphQL query instead of a REST path.',
    {
      ...
      body: { type: 'object', additionalProperties: true, description: 'JSON body for POST/PATCH/PUT, or GraphQL variables.' },
