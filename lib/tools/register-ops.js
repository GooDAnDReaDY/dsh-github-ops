...
export function registerOpsTools({ tool, asRepo, client, liveConfig }) {
  ...
  tool(
    'gh_run_dispatch',
    'Trigger a workflow_dispatch event for a GitHub Actions workflow with inputs. Requires confirm: true.',
    repoParams({
      workflow: { type: 'string', description: 'Workflow file name (e.g. "ci.yml") or numeric ID.' },
      ref: { type: 'string', description: 'Branch or tag ref (e.g. "main").' },
      inputs: { type: 'object', additionalProperties: true, description: 'Input values configured in the workflow.' },
      confirm: { type: 'boolean', description: 'Must be true to dispatch.' },
    }),
    async (args) => {
      if (args.confirm !== true) throw new Error('dispatching a workflow run requires confirm: true')
      return withRepo(args, async ({ owner, repo }) => dispatchWorkflow(await client(), {
        owner, repo, workflow: args.workflow, ref: args.ref, inputs: args.inputs,
      }))
    },
  )
  ...
  tool(
    'gh_run_log_summary',
    'Get a compact summary of failed step logs for a run or job (cleans ANSI codes, strips tokens, returns last lines). Read-only.',
    repoParams({
      runId: { type: 'number', description: 'Run ID (if jobId omitted, inspects failed jobs in the run).' },
      jobId: { type: 'number', description: 'Direct Job ID.' },
      maxLines: { type: 'number', description: 'Maximum log lines to return (default 100, max 250).' },
    }),
    async (args) => withRepo(args, async ({ owner, repo }) => getJobLogSummary(await client(), {
      owner, repo, runId: args.runId, jobId: args.jobId, maxLines: args.maxLines,
    })),
  )
