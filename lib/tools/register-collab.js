...
export function registerCollabTools({ tool, asRepo, client, liveConfig, reviewRules }) {
  const repoParams = (extra = {}) => ({ ...REPOSITORY, ...extra })
  const rules = () => (typeof reviewRules === 'function' ? reviewRules() : undefined)
  ...
  tool(
    ...
    async (args) => {
      const { owner, repo } = await asRepo(args, liveConfig())
      return reviewPull(await client(), { owner, repo, number: args.number, maxDiffChars: args.maxDiffChars, rules: rules() })
    },
  )
  ...
  tool(
    ...
    async (args) => {
      const { owner, repo } = await asRepo(args, liveConfig())
      return ciRun(await client(), { owner, repo, number: args.number, maxDiffChars: args.maxDiffChars, rules: rules() })
    },
  )

  tool(
    'pr_review_threads',
    'List review discussion threads of a pull request with their comments and resolved status. Read-only.',
    repoParams({
      number: { type: 'number', description: 'Pull request number.' },
      unresolvedOnly: { type: 'boolean', description: 'If true, returns only unresolved threads (default false).' },
    }),
    async (args) => {
      const { owner, repo } = await asRepo(args, liveConfig())
      return listReviewThreads(await client(), {
        owner, repo,
        number: args.number || args.pullNumber,
        unresolvedOnly: Boolean(args.unresolvedOnly),
      })
    },
  )
  ...
  tool(
    'pr_thread_reply',
    'Reply to a review discussion thread in a pull request (by threadId or commentId). Requires confirm: true.',
    repoParams({
      number: { type: 'number', description: 'Pull request number (required when replying by commentId).' },
      threadId: { type: 'string', description: 'GraphQL ID of the review thread (from pr_review_threads).' },
      commentId: { type: 'number', description: 'Database ID of the comment being replied to.' },
      body: { type: 'string', description: 'Reply comment body.' },
      confirm: { type: 'boolean', description: 'Must be true to post reply.' },
    }),
    async (args) => {
      if (args.confirm !== true) throw new Error('replying to a review thread requires confirm: true')
      const { owner, repo } = await asRepo(args, liveConfig())
      return replyReviewThread(await client(), {
        owner, repo,
        number: args.number || args.pullNumber,
  ...
  tool(
    'pr_thread_resolve',
    'Resolve or unresolve a review discussion thread in a pull request. Requires confirm: true.',
    repoParams({
      threadId: { type: 'string', description: 'GraphQL ID of the review thread (from pr_review_threads).' },
      resolve: { type: 'boolean', description: 'true to resolve (default), false to unresolve.' },
      confirm: { type: 'boolean', description: 'Must be true to resolve thread.' },
    }),
    async (args) => {
      if (args.confirm !== true) throw new Error('resolving a review thread requires confirm: true')
      const { owner, repo } = await asRepo(args, liveConfig())
      return resolveReviewThread(await client(), {
        owner, repo,
        threadId: args.threadId,
