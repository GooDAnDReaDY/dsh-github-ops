 ...
 */
export function decide({ exec, allowedActions, autoApprove, unattended = false } = {}) {
  ...
  if (unattended && !DESTRUCTIVE_ACTIONS.has(action) && Array.isArray(autoApprove) && autoApprove.includes(action)) {
    return { kind: 'allow', action }
  }
  const destructive = DESTRUCTIVE_ACTIONS.has(action) ? ' This cannot be undone here.' : ''
  return { kind: 'ask', action, reason: `dsh-github-ops: ${askReason(exec)}.${destructive}` }
}
