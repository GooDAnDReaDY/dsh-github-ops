...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      tokenEnvHint: 'Name of the DSH credential that holds the GitHub token. The token itself is never stored here.',
    ...
    const zh = {
      ...
      tokenEnv: '凭据名称',
      tokenEnvHint: '存放 GitHub 令牌的 DSH 凭据名称；令牌本身不会保存在这里。',
    ...
    function GitHubOpsCard(props) {
      ...
      const save = async () => {
        ...
        setSaving(true)
        const failed = []
        const keys = ['tokenEnv', 'defaultRepository', 'baseUrl', 'timeoutMs']
        for (const key of keys) {
          ...
          if (key === 'timeoutMs') {
            ...
            if (!Number.isFinite(numeric) || numeric <= 0) {
              failed.push(`${t('timeoutMs')}: ${t('invalidNumber')}`)
              continue
            }
            next = numeric
          }
      ...
      const field = (key, labelKey, hintKey) => React.createElement('div', { className: 'gho-field' },
        ...
        React.createElement('input', {
          id: `${NS}-${key}`,
          className: 'gho-input',
          type: key === 'timeoutMs' ? 'number' : 'text',
      ...
      const body = React.createElement('div', { className: 'gho-body' },
        ...
        field('timeoutMs', 'timeoutMs', 'timeoutMsHint'),
